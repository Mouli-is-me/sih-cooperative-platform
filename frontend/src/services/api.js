...
// Live Mode: Connects to Node.js / Express REST API
...
export const api = {
  ...
  async getHardwareDevice(deviceCode = "DEV-001") {
    ...
    if (!res.ok) {
      throw new Error(
        data.error?.message || `Unable to load hardware device (${res.status})`,
      );
    }
  ...
  async setHardwareCommand(deviceCode = "DEV-001", state, jobId = null) {
    ...
    if (!res.ok) {
      throw new Error(
        data.error?.message || `Unable to control hardware (${res.status})`,
      );
    }
  ...
  async sendJobToHardware(jobId, deviceCode = "DEV-001") {
    if (!jobId) {
      throw new Error("A job ID is required to send a job to the device.");
    }
  ...
  async acceptHardwareJob(jobId, deviceCode = "DEV-001") {
    if (!jobId) {
      throw new Error("A job ID is required to accept the hardware job.");
    }
