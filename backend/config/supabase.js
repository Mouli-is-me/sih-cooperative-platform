...
if (isSupabaseConfigured()) {
  console.log("[CO-OP OS DB] Supabase PostgreSQL Client Initialized Successfully");
} else {
  console.log("[CO-OP OS DB Info] Supabase credentials not found in env. Running in dynamic fallback mode.");
}
...
export const updateWorkerStatusInSupabase = async (id, status, isAvailable, availabilityText) => {
  if (!supabase) return null;
  try {
...
export const updateServiceRequestStatusInSupabase = async (id, status) => {
  if (!supabase) return null;
  try {
    const { data, error } = await supabase
...
export const getCooperativeAnalyticsFromSupabase = async () => {
  if (!supabase) return null;
  try {
    const { data, error } = await supabase.from("cooperative_analytics").select("*").eq("id", "current_analytics").single();
    if (error || !data) return null;
    return {
